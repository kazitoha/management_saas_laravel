                        ...
                        <div class="p-4">
                            ...
                            @if ($todayRecord)
                                ...
                                <p class="mb-1 text-sm">
                                    ...
                                    {{ $todayRecord->check_in_at ? new \DateTime($todayRecord->check_in_at)->format('H:i') : '—' }}
                                </p>
                                @if ($todayRecord->check_out_at)
                                    <p class="mb-1 text-sm">
                                        <span class="font-semibold text-slate-700">Check-out:</span>
                                        {{ $todayRecord->check_out_at ? new \DateTime($todayRecord->check_out_at)->format('H:i') : '—' }}
                            ...
                            @else
                                ...
                                <form id="attendanceForm" method="POST"
                                    action="{{ $todayRecord && !$todayRecord->check_out_at ? route('attendance.checkout') : route('attendance.checkin') }}">
                                    @csrf
                                    ...
                                    <p class="mt-2 text-[11px] leading-snug text-slate-500">
                                        Location is required. If blocked, enable location permission in your browser
                                        settings
                                        and tap retry.<br>
                                ...
                                <table class="min-w-full border-collapse text-xs sm:text-sm">
                                    <thead>
                                        <tr class="bg-slate-50 text-slate-700">
                                            ...
                                            <th class="border border-slate-200 px-3 py-2 text-left font-semibold">Check-in
                                            </th>
                                            <th class="border border-slate-200 px-3 py-2 text-left font-semibold">Check-out
                                            </th>
                                            <th class="border border-slate-200 px-3 py-2 text-left font-semibold">Duration
                                            </th>
                                            <th class="border border-slate-200 px-3 py-2 text-left font-semibold">Location
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @forelse ($recent as $rec)
                                            <tr class="odd:bg-white even:bg-slate-50">
                                                <td class="border border-slate-200 px-3 py-2 whitespace-nowrap">
                                                    {{ $rec->check_in_at ? \Carbon\Carbon::parse($rec->check_in_at)->format('Y-m-d') : '—' }}
                                                </td>
                                                <td class="border border-slate-200 px-3 py-2 whitespace-nowrap">
                                                    ...
                                                    <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
                                                        @if ($rec->check_in_lat && $rec->check_in_lng)
                                                            <a href="https://maps.google.com/?q={{ $rec->check_in_lat }},{{ $rec->check_in_lng }}"
                                                                target="_blank"
                                                                class="inline-flex items-center justify-center gap-2 rounded-md border border-sky-200 bg-sky-50 px-3 py-1.5 text-xs font-medium text-sky-700 hover:bg-sky-100 hover:border-sky-300">
                                                                <!-- optional icon -->
                                                        ...
                                                        @if ($rec->check_out_lat && $rec->check_out_lng)
                                                            <a href="https://maps.google.com/?q={{ $rec->check_out_lat }},{{ $rec->check_out_lng }}"
                                                                target="_blank"
                                                                class="inline-flex items-center justify-center gap-2 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-medium text-emerald-700 hover:bg-emerald-100 hover:border-emerald-300">
                                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4"
                                                                    fill="none" viewBox="0 0 24 24"
                                                                    stroke="currentColor">
                                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        ...
                                                        @if (!($rec->check_in_lat && $rec->check_in_lng) && !($rec->check_out_lat && $rec->check_out_lng))
